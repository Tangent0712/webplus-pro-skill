...
> 完整原因 + **可照抄的 CSS/JS 配方**（搜索组件、分页组件、导航高亮、面包屑、侧栏、前端兜底渲染、
> 模板 HTML 陷阱、后台操作）见 **`reference/pitfalls.md`**。下表只做快速索引。

| 症状 | 根因 | 处理 |
|---|---|---|
| 前台出现字面量 `null` | `simpleList` 未绑「信息来源」 | 绑栏目，或前端兜底 |
| 列表页显示的是**一篇文章正文**而不是列表 | 该栏目**已发布文章只有 1 篇**（系统自动按单篇渲染）；或栏目本就是单页栏目 | 再发布 ≥2 篇即恢复列表；单页栏目（简介/联系我们）属预期 |
| 进入**父栏目**列表页后，侧栏/导航没有任何高亮 | 父栏目 URL 实际渲染的是**第一个子栏目**的内容，而侧栏链接指向子栏目 URL，按 pathname 匹配不上 | 建 `parentUrl -> firstChildUrl` 映射后再匹配；一级导航建议直接指向第一个子栏目 |
